...
parser.add_argument('--warmup_ep', default=30, type=int)
...
# Training
def train(epoch, net, ema_net, net2, ema_net2, labeled_trainloader, confi_weight1, confi_weight2):
    net.train()
    net2.train()
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        labels_x2 = torch.zeros(batch_size, args.num_class).cuda().scatter_(1, labels_x.cuda().view(-1, 1), 1)
        labels_x = torch.zeros(batch_size, args.num_class).cuda().scatter_(1, labels_x.cuda().view(-1, 1), 1)
...
def warmup(epoch, net, ema1, net2, ema2, dataloader):
    net.train()
    net2.train()
    for batch_idx, (inputs_w, inputs_s, labels, index, _) in enumerate(dataloader):
...
def cleanset_eval(teacher, student, true_label, targets_list):
    ...
    precision_s = (targets_list[idx_chosen_student] == true_label[
        idx_chosen_student]).sum() * 100 / idx_chosen_student.sum()
    recall_s = (targets_list[idx_chosen_student] == true_label[idx_chosen_student]).sum() * 100 / (
                targets_list == true_label).sum()

    # teacher net
    idx_chosen_teacher = teacher == 1
    precision_t = (targets_list[idx_chosen_teacher] == true_label[
        idx_chosen_teacher]).sum() * 100 / idx_chosen_teacher.sum()
    recall_t = (targets_list[idx_chosen_teacher] == true_label[idx_chosen_teacher]).sum() * 100 / (
                targets_list == true_label).sum()
...
def eval_train(model, rho, id):
    ...
    num_class = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets, index, label) in enumerate(eval_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
...
optimizer1 = optim.SGD([{'params': dualnet.net1.parameters()},
                        {'params': dualnet.net2.parameters()}], lr=args.lr, momentum=0.9, weight_decay=5e-4)
...
for epoch in range(args.num_epochs):
    ...
    if epoch < warm_up:
        ...
        train(epoch, dualnet.net1, ema_net.net1, dualnet.net2, ema_net.net2, total_trainloader, confi_weight1,
              confi_weight2)
        scheduler1.step()
        if epoch < args.num_epochs - args.start_expand:
            scheduler2.step()
