...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log='', q_pre=[], q_label=[]):
        ...
        if self.mode=='test':
            ...
            if os.path.exists(noise_file):
                ...
                if dataset == "cifar100" and noise_mode == "asym":
                    noise_label = self.asymmetric_noise(np.array(train_label)).tolist()
                else:
            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    noise_label[q_pre] = q_label[q_pre]
...
class cifar_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[], q_pre=[], q_label=[]):
        if mode=='warmup':
            all_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=[self.weak, self.strong, self.normalize], mode="all",noise_file=self.noise_file)
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir,\
                                            ...
                                            noise_file=self.noise_file, pred=pred, probability=prob,log=self.log,q_pre=q_pre, q_label=q_label)
